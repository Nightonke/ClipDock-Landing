...
const publishedMedia = JSON.parse(execFileSync('curl', ['--fail', '--silent', '--show-error', '--retry', '2', '--max-time', '30', `${config.mediaBaseUrl}/manifest.json`], { encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 }));
...
await Promise.all(Array.from({ length: 8 }, async () => {
  while (derivatives.length) {
    ...
    const { stdout } = await run('curl', ['--fail', '--silent', '--show-error', '--retry', '2', '--max-time', '60', `${config.mediaBaseUrl}/assets/${file.path}`], { encoding: 'buffer', maxBuffer: 16 * 1024 * 1024 });
