...
构建中已启用的语言为 `en-US`、`zh-Hans`、`zh-Hant`、`ja`、`ko`、`es`、`pt-BR` 和 `de`。英文使用 `/`，简体中文使用 `/zh-Hans/`，繁体中文使用 `/zh-Hant/`，日语使用 `/ja/`，韩语使用 `/ko/`，西班牙语使用 `/es/`，巴西葡萄牙语使用 `/pt-BR/`，德语使用 `/de/`，首页、41 篇教程及站内法律页面共用模板。各语言的部署状态见对应交付记录。不会按设备语言自动跳转。
...
`zh-Hant/` 已补齐当前完整内容，采用台湾常用表达，操作名按 App 繁中实际值保留。术语差异、验证与部署状态见 [繁中交付记录](localization-reports/zh-Hant.md)。英文 `site-defaults.ts` 保存站点兼容默认值，并非其他语言的译文来源。`ja/` 提供完整日语内容，按钮名称按 App 日语实际值保留，疑点和验证范围见 [日语交付记录](localization-reports/ja.md)。`ko/` 提供完整韩语内容，使用韩国常用说明文和礼貌表达；操作名称对照 App 韩语实际值，标题按词换行。术语疑点、验证和未覆盖项见 [韩语交付记录](localization-reports/ko.md)。`es/` 提供跨地区西班牙语完整文案，正文使用统一的 tú 称呼，按钮按 App 实际值保留，完整范围与验证结果见 [西语交付记录](localization-reports/es.md)。`pt-BR/` 提供完整巴西葡萄牙语内容，采用巴西常用表达与 você 称呼，按钮和工具名称以 App 巴葡实际值为准，保留与葡萄牙葡语的地区区别。术语、验证和交付状态见 [巴葡交付记录](localization-reports/pt-BR.md)。`de/` 提供完整德语内容，采用通用德语与统一的 `du` 称呼，操作名称按 App 德语实际值保留，并针对复合词与法律页长标题调整排版。术语疑点、验证和交付状态见 [德语交付记录](localization-reports/de.md)。当前隐私政策和服务条款由 `legal.ts` 注册站内正文，新增语言需同步翻译并纳入 sitemap；新增语言指南中的外链法律页描述仅适用于旧基准。
