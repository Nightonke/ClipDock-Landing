...
已发布语言为 `en-US` 和 `zh-Hans`。英文使用 `/`，简体中文使用 `/zh-Hans/`，首页与 41 篇教程均共用模板。不会按设备语言自动跳转。
...
`zh-Hant/` 只保存已有的未发布内容，不代表繁体官网已完成。正式启用前必须补齐当前首页、教程和人工评审。英文 `site-defaults.ts` 保存站点兼容默认值，并非其他语言的译文来源。外链隐私政策与服务条款由外部页面管理，本仓库不保存法律正文副本。
