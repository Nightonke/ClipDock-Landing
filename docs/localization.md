...
构建中已启用的语言为 `en-US`、`zh-Hans`、`zh-Hant`、`ja`、`ko`、`es` 和 `pt-BR`。英文使用 `/`，简体中文使用 `/zh-Hans/`，繁体中文使用 `/zh-Hant/`，日语使用 `/ja/`，韩语使用 `/ko/`，西班牙语使用 `/es/`，巴西葡萄牙语使用 `/pt-BR/`，首页、41 篇教程及站内法律页面共用模板。各语言的部署状态见对应交付记录。不会按设备语言自动跳转。
